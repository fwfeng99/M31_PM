...
class MCI_Field(MCI):
    def __init__(self, field_ra_list, field_dec_list, field_index, blocks_num):
        ...
        self.coord_ideal = function.cel_ideal_coord(
            self.coord[0], self.coord[1], self.M31_c.ra.value, self.M31_c.dec.value
        )
        self.rho = np.arctan(
            (self.coord_ideal[0] ** 2 + self.coord_ideal[1] ** 2) ** 0.5
        )
        ...
        self.coord_ideal_part = function.cel_ideal_coord(
            np.array(self.coord_part)[:, 0],
            np.array(self.coord_part)[:, 1],
            self.M31_c.ra.value,
            self.M31_c.dec.value,
        )

        self.rho_part = np.arctan(
            (self.coord_ideal_part[0] ** 2 + self.coord_ideal_part[1] ** 2) ** 0.5
        )
        self.phi_part = np.pi - np.arctan2(
            self.coord_ideal_part[1], self.coord_ideal_part[0]
        )
...
class PM_COM_fit(object):
    ...
    def v_para_factor(self, *args):
        if len(args) == 1:
            ...
            p_t = (
                self.field.phi_part[args] - self.field.M31_PA * np.pi / 180 - np.pi / 2
            )  # phi substracts theta(PA)
        else:
            rho = self.field.rho
            phi = self.field.phi
            p_t = (
                self.field.phi - self.field.M31_PA * np.pi / 180 - np.pi / 2
            )  # phi substracts theta(PA)
...
# path_pm = "M31_PM/PM_calculate/Data/PM_result_submit"
path_pm = "M31_PM/PM_calculate/Data/PM_result_0320"
x = []
y = []
R_uv = []
field_index_list = range(field_index_start, field_index_end + 1)
# field_index_list = [1, 2, 3, 4]
field_index_list = [5, 6, 7, 8]
field_index_list = [1, 5, 2, 6]
field_index_list = [3, 7, 4, 8]
# field_index_list = [2, 6, 4, 8]
# field_index_list = [1, 5, 7, 3]
field_index_list = [2, 4, 1]
# field_index_list = [2, 3, 4]
field_index_list = [1, 2]
field_index_list = [3, 4]
...
for field_index in field_index_list:
    ...
    # 观测数据和自变量
    files = glob.glob(
        os.path.join(path_pm, f"*{field_index}*{obs_times}_{blocks_num ** 2}_*.csv")
    )
    for file in files:
        # print(file)
        ...
        pm_w = (
            -pm_ra_obs
            * np.cos(np.deg2rad(mci_field.coord_part[field_part_index - 1][1]))
            / 1000
        )
...
for i in range(sta_times):
    ...
    initial_guess = [
        np.random.normal(a, 100),
        np.random.normal(b, 1),
        np.random.normal(c, 20),
    ]  # 请提供适当的初始值

    result = minimize(
        error_function, initial_guess, args=(x1, x2, x3, x4, x5, x6, x7, y1, y2)
    )
...
index = (vt > 0) & (theta < 0) & (theta > -2 * np.pi) & (Vr < -100)
...
for field_index in field_index_list:
    ...
    for file in files:
        ...
        pm_w = -pm_ra_ideal * np.cos(
            np.deg2rad(mci_field.coord_part[field_part_index - 1][1])
        )
        pm_n = pm_dec_ideal
        pm_w = pm_w[index]
        pm_n = pm_n[index]
        ...
        # R_uv_temp = PM_COM.u_v()
        y1_temp = (
            vp_temp[0] * vt * np.cos(theta)
            + vp_temp[1] * vt * np.sin(theta)
            + vp_temp[2]
            + vp_temp[3] * Vr
        )
        y2_temp = (
            vp_temp[4] * vt * np.cos(theta)
            + vp_temp[5] * vt * np.sin(theta)
            + vp_temp[6] * Vr
        )
        ...
        sta_pm.append(
            [
                np.mean(pm_w),
                np.mean(pm_wn_temp[0, :]),
                np.std(pm_wn_temp[0, :]),
                np.mean(pm_n),
                np.mean(pm_wn_temp[1, :]),
                np.std(pm_wn_temp[1, :]),
            ]
        )
...
# print(np.mean(vt), '&', np.std(vt), '&', np.mean(theta), '&', np.std(theta), '&', np.mean(Vr), '&', np.std(Vr))
print(field_index_list)
print(
    field_index_list,
    "& {:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f}".format(
        np.mean(vt), np.std(vt), np.mean(theta), np.std(theta), np.mean(Vr), np.std(Vr)
    ),
)
