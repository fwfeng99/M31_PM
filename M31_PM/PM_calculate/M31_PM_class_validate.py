...
sys.path.extend(
    [
        "D:\\repos\\PycharmProjects\\M31_kinematics_validate",
        "D:/repos/PycharmProjects/M31_kinematics_validate",
    ]
)
import function
...
class MCI_Field(MCI):
    def __init__(self, field_ra_list, field_dec_list, field_index):
        ...
        self.coord = np.array(
            [field_ra_list[field_index - 1], field_dec_list[field_index - 1]]
        )
        self.M31_c = SkyCoord.from_name("M31")
        self.coord_cel = function.cel_ideal_coord(
            self.coord[0], self.coord[1], self.M31_c.ra.value, self.M31_c.dec.value
        )
...
class Cat_process(object):
    ...
    def cal_cat_err(self, cat_err_f=1 / 6.6):
        ...
        self.ra_err = (
            np.random.normal(
                loc=np.zeros(num_stars), scale=self.cat_file["e_ra"], size=num_stars
            )
            * cat_err_f
        )
        self.dec_err = (
            np.random.normal(
                loc=np.zeros(num_stars), scale=self.cat_file["e_dec"], size=num_stars
            )
            * cat_err_f
        )
        self.pm_ra_err = (
            np.random.normal(
                loc=np.zeros(num_stars), scale=self.cat_file["e_pm_ra"], size=num_stars
            )
            / np.cos(self.cat_file["dec"] * np.pi / 180)
            * cat_err_f
        )
        self.pm_dec_err = (
            np.random.normal(
                loc=np.zeros(num_stars), scale=self.cat_file["e_pm_dec"], size=num_stars
            )
            * cat_err_f
        )
    ...
    def cal_real_pm(self):
        self.pm_ra_real = (
            self.cat_file["pm_ra"] / np.cos(self.cat_file["dec"] * np.pi / 180)
            + self.pm_ra_err
        )
...
class Model(object):
    ...
    def add_cat_err_cel(self, ra_dec, ra_dec_err, pm_err, year):
        ...
        return (
            (ra_dec * 3600 * 1000 + ra_dec_err + year * pm_err) / 3600 / 1000
        )  # unit is degree
    ...
    def cal_chip_cat_model(self):
        ...
        for i, year in enumerate(self.epoch.years):
            ...
            for obs_t in range(obs_times):
                ...
                ra_cat_year = self.cal_cel(
                    self.cat.cat_file["ra"],
                    self.cat.cat_file["pm_ra"]
                    / np.cos(self.cat.cat_file["dec"] * np.pi / 180),
                    year,
                )
                dec_cat_year = self.cal_cel(
                    self.cat.cat_file["dec"], self.cat.cat_file["pm_dec"], year
                )
                ...
                # )
                # dec_cat_year = self.cal_cel(
                #     self.cat.cat_file["dec"], self.cat.pm_dec_real, year
                # )

                xi_cat_year, eta_cat_year = function.cel_ideal_coord(
                    ra_cat_year, dec_cat_year, self.field.coord[0], self.field.coord[1]
                )

                # xi corresponds to x and ra, eta corresponds to y and dec
                # 1, add Gaia catalogue error (include magnitude error)
                ra_cat_year_obs = self.add_cat_err_cel(
                    ra_cat_year, self.cat.ra_err, self.cat.pm_ra_err, year
                )
                dec_cat_year_obs = self.add_cat_err_cel(
                    dec_cat_year, self.cat.dec_err, self.cat.pm_dec_err, year
                )
                [xi_cel_cat_year_obs, eta_cel_cat_year_obs] = function.cel_ideal_coord(
                    ra_cat_year_obs,
                    dec_cat_year_obs,
                    self.field.coord[0],
                    self.field.coord[1],
            ...
            chip_model = self.cal_chip_one_model(
                xi_cat_year, eta_cat_year, x_year_obs, y_year_obs
            )
            chip_model_list.append(chip_model)

            start, end = i * num_stars, (i + 1) * num_stars
            mci_cel_cat[start:end, 0:4] = np.vstack(
                (ra_cat_year, dec_cat_year, xi_cat_year, eta_cat_year)
            ).T
    ...
    def cal_chip_cat_model_new(self):
        ...
        for i, year in enumerate(years):
            ra_cat_year = self.cal_cel(
                self.cat.cat_file["ra"], self.cat.pm_ra_obs, year
            )
            dec_cat_year = self.cal_cel(
                self.cat.cat_file["dec"], self.cat.pm_dec_obs, year
            )
    ...
    def chip_cel_obs(self):
        ...
        for i in range(self.epoch.num_years):
            ...
            fit_cof = np.array([[1, 0, 0], [0, 1, 0]])

            x_temp = x_obs[i * num_stars : (i + 1) * num_stars]
            y_temp = y_obs[i * num_stars : (i + 1) * num_stars]

            xi_eta_obs_cel = (
                np.dot(fit_cof, np.array([x_temp, y_temp, np.ones_like(x_temp)]))
                / self.field.f_p
            )
            [ra_obs_temp, dec_obs_temp] = function.ideal_cel_coord(
                xi_eta_obs_cel[0, :],
                xi_eta_obs_cel[1, :],
                self.field.coord[0],
                self.field.coord[1],
    ...
    def cal_obs_pm(self, mid_num=4):
        ...
        (
            ...
            pm_ra_obs_err,
            pm_dec_obs_err,
        ) = np.zeros((6, num_stars))

        for i in range(num_stars):
            y_ra_t = np.array(
                [self.cat.ra_obs[i + j * num_stars] for j in range(num_years)]
            )
            y_dec_t = np.array(
                [self.cat.dec_obs[i + j * num_stars] for j in range(num_years)]
            )
...
class M31_PM(object):
    ...
    def cal_star_coord(self):
        ...
        [
            ...
            self.dec_field,
            self.field_flag,
        ] = function.local_cel_ideal_coord(
            self.star_local["x"],
            self.star_local["y"],
            self.field.M31_dis,
            self.field.M31_inc,
            self.field.M31_PA,
            self.field.M31_c.ra.degree,
            self.field.M31_c.dec.degree,
            self.field.coord,
            self.field.mci_field_size,
        )
        self.num_stars = len(self.ra_field)

        [self.xi_field, self.eta_field] = function.cel_ideal_coord(
            self.ra_field,
            self.dec_field,
            self.field.M31_c.ra.degree,
            self.field.M31_c.dec.degree,
    ...
    def cal_star_v(self):
        ...
        phi = self.phi_field
        p_t = (
            self.phi_field - self.field.M31_PA * np.pi / 180 - np.pi / 2
        )  # phi substracts theta(PA)
        ...
        v2 = (
            vt * np.cos(rho) * np.cos(phi - theta)
            - v_sys * np.sin(rho)
            + np.sin(i)
            * np.cos(p_t)
            * (np.cos(i) * np.sin(rho) + np.sin(i) * np.cos(rho) * np.sin(p_t))
            / (np.cos(i) ** 2 * np.cos(p_t) ** 2 + np.sin(p_t) ** 2) ** 0.5
            * Vr
        )
        v3 = (
            -vt * np.sin(phi - theta)
            - (np.cos(i) ** 2 * np.cos(p_t) ** 2 + np.sin(p_t) ** 2)
            / (np.cos(i) ** 2 * np.cos(p_t) ** 2 + np.sin(p_t) ** 2) ** 0.5
            * Vr
        )

        cos_T = (
            np.sin(dec) * np.cos(dec_c) * np.cos(ra - ra_c)
            - np.cos(dec) * np.sin(dec_c)
        ) / np.sin(rho)
        sin_T = np.cos(dec_c) * np.sin(ra - ra_c) / np.sin(rho)
        D_f = (np.cos(i) * np.cos(rho) - np.sin(i) * np.sin(rho) * np.sin(p_t)) / (
            D0 * np.cos(i)
        )
        ...
        for i in range(len(v2)):
            self.vwn[:, i] = (
                D_f[i] * rot_f[:, :, i] @ self.v23[:, i] / 4.740470463533348 * 1000
            )
    ...
    def cal_ideal_cat(self):
        ...
        # ideal catalogue
        for i, year in enumerate(years):
            ra_field = self.cal_cel(
                self.ra_field,
                -self.vwn[0, :] / np.cos(self.dec_field * np.pi / 180),
                year,
            )
            dec_field = self.cal_cel(self.dec_field, self.vwn[1, :], year)

            [xi_field, eta_field] = function.cel_ideal_coord(
                ra_field, dec_field, self.field.coord[0], self.field.coord[1]
            )

            start, end = i * num_stars, (i + 1) * num_stars
            field_cat[start:end, 0:4] = np.vstack(
                (xi_field, eta_field, ra_field, dec_field)
            ).T
    ...
    def cal_ideal_pm(self):
        ...
        self.ideal_cat = pd.DataFrame(
            np.array([ra_t0_cat, dec_t0_cat, self.pm_ra_ideal, self.pm_dec_ideal]).T,
            columns=columns,
        )
    ...
    def cal_obs_pm(self, mid_num=4):
        ...
        for i in range(num_stars):
            y_ra_t = np.array(
                [self.ra_obs[i + j * num_stars] for j in range(num_years)]
            )
            y_dec_t = np.array(
                [self.dec_obs[i + j * num_stars] for j in range(num_years)]
            )
    ...
    def obs_pm_part(self, blocks_num_list, blocks_res_list):
        for blocks_num, blocks_res in zip(blocks_num_list, blocks_res_list):
            blocks_index = np.linspace(-blocks_num + 1, blocks_num - 1, blocks_num)
            blocks_index_list = [[j, i] for i in blocks_index for j in blocks_index]

            mci_field_size_part = self.field.mci_field_size / blocks_num * (np.pi / 180)
            coord_part_list = [
                function.ideal_cel_coord(
                    x * mci_field_size_part / 2,
                    y * mci_field_size_part / 2,
                    self.field.coord[0],
                    self.field.coord[1],
                )
                for x, y in blocks_index_list
            ]

            for coord_part, res_part in zip(coord_part_list, blocks_res):
                [xi, eta] = function.cel_ideal_coord(
                    self.ra_field, self.dec_field, coord_part[0], coord_part[1]
                )
                index_flag = (np.abs(xi) < mci_field_size_part / 2) & (
                    np.abs(eta) < mci_field_size_part / 2
                )

                res_part.append(
                    [
                        np.mean(self.pm_ra_ideal[index_flag]),
                        np.mean(self.pm_ra_obs[index_flag]),
                        np.mean(self.pm_dec_ideal[index_flag]),
                        np.mean(self.pm_dec_obs[index_flag]),
                    ]
                )
...
field_index = 8
field_ra_list = np.array(
    [12.28574129, 11.22870306, 9.15566471, 10.09915259, 12.064, 10.331, 9.185, 11.250]
)
field_dec_list = np.array(
    [42.75043505, 40.95362697, 39.76631034, 41.6017492, 41.865, 40.195, 40.435, 42.480]
)
# blocks_num = 2
...
blocks_num_list = [1, 2, 3, 4, 5, 6, 7]
blocks_res_list = [
    [[] for _ in range(blocks_num**2)] for blocks_num in blocks_num_list
]
...
for model_list_num in range(len(Chip_list)):
    ...
    m31_pm.obs_pm_part(blocks_num_list, blocks_res_list)

columns = ["ra_pm_ideal", "ra_pm_obs", "dec_pm_ideal", "dec_pm_obs"]

path = "M31_PM/PM_calculate/Data/PM_result_0320"

for blocks_num, blocks_res in zip(blocks_num_list, blocks_res_list):
    for part_index, res_part in zip(range(1, blocks_num**2 + 1), blocks_res):
        res_part = pd.DataFrame(res_part, columns=columns)
        filename = (
            f"res_{field_index}_{obs_times}_{blocks_num ** 2}_{part_index}_val.csv"
        )
        res_part.to_csv(os.path.join(path, filename), index=False)


# ----------------------------------------------------------------------------------------------------------------------
# write PM ideal result
# path = 'M31_PM/PM_calculate/Data/PM_result_ideal'
# filename = f'res_ideal_{field_index}.csv'
# m31_pm.ideal_cat.to_csv(os.path.join(path, filename), index=False)
#
# blocks_num = 2
# blocks_index = np.linspace(-blocks_num + 1, blocks_num - 1, blocks_num)
# blocks_index_list = [[j, i] for i in blocks_index for j in blocks_index]
#
# mci_field_size_part = mci.mci_field_size / blocks_num * (np.pi / 180)
# coord_part_list = [function.ideal_cel_coord(x * mci_field_size_part / 2, y * mci_field_size_part / 2, mci_field.coord[0], mci_field.coord[1]) for x, y in blocks_index_list]
#
# for part_index, coord_part in enumerate(coord_part_list):
#     [xi, eta] = function.cel_ideal_coord(m31_pm.ra_field, m31_pm.dec_field, coord_part[0], coord_part[1])
#     field_size_part_rad = m31_pm.field.mci_field_size / 2 * (np.pi / 180)
#     index_flag = (np.abs(xi) < field_size_part_rad / 2) & (np.abs(eta) < field_size_part_rad / 2)
#     columns = ['ra_ideal', 'dec_ideal', 'ra_pm_ideal', 'dec_pm_ideal']
#     filename = f'res_ideal_{field_index}_{part_index + 1}.csv'
#     pd.DataFrame(np.array([m31_pm.ra_field[index_flag], m31_pm.dec_field[index_flag],
#                            m31_pm.pm_ra_ideal[index_flag], m31_pm.pm_dec_ideal[index_flag]]).T,
#                  columns=columns).to_csv(os.path.join(path, filename), index=False)
...
#
#     path = 'M31_PM/PM_calculate/Data/PM_result_comp'
#     files = glob.glob(os.path.join(path, f"*{field_index}*{obs_times}_{9}_*.csv"))
#     for file in files:
