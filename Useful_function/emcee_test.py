...
def ln_likelihood(params, x1, x2, x3, x4, y1, y2, sigma1, sigma2):
    ...
    a, b = params
    model_y1 = x1 * a * np.cos(b) + x2 + x3
    model_y2 = -a * np.sin(b) + x4
...
def ln_prior(params):
    # a, b, c = params
    # if -200 < a < 200 and 0 < b < 2 * np.pi and -300 < c < 300:
    a, b = params
    if -200 < a < 200 and -2 * np.pi < b < 2 * np.pi:
        return 0.0  # 先验概率为常数
...
a_median, b_median = np.percentile(samples, 50, axis=0)
